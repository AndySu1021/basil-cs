<template>
  <AppDialog ref="dialog" title="編輯常見問題" width="500px" top="5vh">
    <div slot="body">
      <el-form
          ref="dialogForm"
          :model="formData"
...
import {apiGetFAQDetail, apiUpdateFAQ} from "@/api/faq";
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        const { data } = await apiGetFAQDetail(this.faqId)
    ...
    submitForm() {
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          try {
            await apiUpdateFAQ(this.faqId, this.formData)
