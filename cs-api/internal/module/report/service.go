...
func (s *service) ListDailyTagReport(ctx context.Context, startDate, endDate time.Time) ([]internal.DailyTagReportColumn, map[string]map[string]int32, error) {
	tags, err := s.repo.GetAllTag(ctx)
	if err != nil {
		return nil, nil, err
	}

	var columns []internal.DailyTagReportColumn
	for _, tag := range tags {
		columns = append(columns, internal.DailyTagReportColumn{
			Label: tag.Name,
