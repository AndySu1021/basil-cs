...
type RedisWorker struct {
	...
	methods map[pkg.ClientEvent]reflect.Method
}
...
func NewRedisWorker(manager *ClientManager, dispatcher *StaffDispatcher, notifier *Notifier) *RedisWorker {
	worker := &RedisWorker{
		...
		methods: make(map[pkg.ClientEvent]reflect.Method),
	}
	worker.registerMethods()
	return worker
}

func (r *RedisWorker) Handle(info pkg.ClientEventInfo) {
...
func (r *RedisWorker) registerMethods() {
	...
	for i := 0; i < t.NumMethod(); i++ {
		...
		r.methods[pkg.ClientEvent(name)] = t.Method(i)
	}
}
...
func (r *EventHandler) CloseRoom(eventInfo pkg.ClientEventInfo) {
...
func (r *EventHandler) AcceptRoom(eventInfo pkg.ClientEventInfo) {
	roomId := *eventInfo.Payload.RoomID
	if member := r.manager.GetMember(roomId); member == nil {
		return
	}
...
func (r *EventHandler) UpdateConfig(eventInfo pkg.ClientEventInfo) {
	config := *eventInfo.Payload.CsConfig
	r.manager.csConfig = config
	r.dispatcher.setMaxMember(config.MaxMember)
}

func (r *EventHandler) TransferRoom(eventInfo pkg.ClientEventInfo) {
	roomId := *eventInfo.Payload.RoomID
...
func (r *EventHandler) MessageReceived(eventInfo pkg.ClientEventInfo) {
	roomId := *eventInfo.Payload.RoomID
